...
def generate_animation_gif(n_layers, duration_sec, material, det_thick_mm, bg_intensity):
    ...
    def update(frame):
        ...
        if n_bg > 0:
            raw_bg = np.random.exponential(scale=0.8, size=n_bg)
            
            #всё, что больше 2.8 МэВ. 
            # 2.6 МэВ - физический предел Таллия-208 + 0.2 МэВ на оптическое размытие детектора
            valid_bg = raw_bg[raw_bg < 2.8]
            
            energies_bg.extend(valid_bg)
        ax2.clear()
...
def render_calibration_tab(det_side_mm, det_thick_mm, bg_intensity):
    ...
    raw_bg_events = np.random.exponential(scale=0.45, size=n_bg)
    ideal_bg = raw_bg_events[raw_bg_events <= 2.6] 
    
    detector_noise = np.random.normal(loc=0.0, scale=0.15, size=len(ideal_bg))
    bg_events = ideal_bg + detector_noise
    bg_events = bg_events[bg_events > 0]
